 ...
 * Used in naive Bayes and decision trees to monitor data statistics on leaves.
 */
class GuassianNumericFeatureClassObserver(val numClasses: Int, val fIndex: Int,
  val numBins: Int = 10) extends NumericFeatureClassObserver with Serializable {
...
object FeatureClassObserver {
  ...
  def createFeatureClassObserver(observer: FeatureClassObserver): FeatureClassObserver = {
    if (observer.isInstanceOf[NominalFeatureClassObserver])
      ...
      new GuassianNumericFeatureClassObserver(observer.
        asInstanceOf[GuassianNumericFeatureClassObserver])
