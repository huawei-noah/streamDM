...
class EvaluateIncremental extends Task {
  ...
  def run(ssc: StreamingContext): Unit = {
    ...
    val learner: NaiveBayesMultinomial = this.learnerOption.getValue()
    learner.init()
    ...
    //Predict and then Train the model
    val predPairs = learner.evaluate(instances)
