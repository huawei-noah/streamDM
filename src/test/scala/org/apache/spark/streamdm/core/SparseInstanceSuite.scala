...
class SparseInstanceSuite extends FunSuite {
  ...
  test("A SparseInstance should return its features given indices") {
  ...
  test("A SparseInstance should return 0.0 for a out of bounds index") {
  ...
  test("A SparseInstance should have a dot operation with another SparseInstance") {
  ...
  test("A SparseInstance should have a dot operation with a DenseInstance") {
    val instance1 = SparseInstance(Array(0,2), Array(1.4,2.1))
    val instance2 = DenseInstance(Array(1.2,2.1,3.2))
    assert(instance1.dot(instance2) == 1.4*1.2+2.1*3.2)
  }

  test("A SparseInstance should be able to add another SparseInstance") {
    val instance1 = SparseInstance(Array(0,2), Array(1.4, 2.1))
  ...
  test("A SparseInstance should be able to add a DenseInstance") {
    val instance1 = SparseInstance(Array(0,2), Array(1.4, 2.1))
  ...
  test("A SparseInstance should be able to append a Feature") {
  ...
  test("A SparseInstance should have a map function for features") {
  ...
  test("A SparseInstance should have a parser in comma separated LibSVM format")
  {
  ...
  test("A SparseInstance should have a .toString override") {
    val instance1 = SparseInstance(Array(1,2), Array(1.4, 1.3))
